...
var pokemonCache *pokecache.Cache[PokemonDetails]

func getPokemonCache() *pokecache.Cache[PokemonDetails] {
	if pokemonCache == nil {
		newCache := pokecache.NewCache[PokemonDetails](30 * time.Second)
		pokemonCache = &newCache
	}
	return pokemonCache
}

func FetchPokemonDetails(nameOrId string) (PokemonDetails, error) {
	url := fmt.Sprintf("https://pokeapi.co/api/v2/pokemon/%s/", nameOrId)

	cachedResponse, ok := getPokemonCache().Get(url)
	...
	if res.StatusCode > 299 {
		return PokemonDetails{}, fmt.Errorf("requesting location areas failed with %s", res.Status)
	}
